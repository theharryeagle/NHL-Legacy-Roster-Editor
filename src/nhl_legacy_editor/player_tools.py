...
TEAM_TABLE_INDEX = 0
PLAYER_RELATION_TABLE_INDEX = 2
PLAYER_BIO_TABLE_INDEX = 7
PLAYER_FLAGS_TABLE_INDEX = 5
PLAYER_RATINGS_TABLE_INDEX = 6
PLAYER_SMALL_LINK_TABLE_INDEX = 8
PLAYER_WIDE_LINK_TABLE_INDEX = 9
PLAYER_INSTANCE_TABLE_INDEX = 3
PLAYER_INSTANCE_AUX_TABLE_INDEX = 4
...
class PlayerSnapshotCache:
    ...
    def get_player_snapshot(self, first_name: str, last_name: str) -> PlayerSnapshot | None:
...
def _table_rows(
    ...
    _table, _fields, rows = access.sample_records(
        db_path,
        table_index,
        limit=table_counts[table_index],
    )
    return rows
...
def get_player_snapshot(db_path: Path, first_name: str, last_name: str) -> PlayerSnapshot | None:
    access = TdbAccess()
    ...
    bio = next(
        (
            row
            for row in bio_rows
            if row.get("PedH") == first_name and row.get("RMbQ") == last_name
        ),
        None,
    )
    if bio is None:
