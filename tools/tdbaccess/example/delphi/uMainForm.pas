...
uses
  ...
  Vcl.ActnList, Vcl.ImgList, Vcl.ComCtrls, Vcl.Grids, Vcl.ExtCtrls, Vcl.ToolWin, System.ImageList;
...
type
  ...
  TfrmMain = class(TForm)
    ...
    procedure itmTableSetCapacityClick(Sender: TObject);
  private
...
procedure TfrmMain.itmTableSetCapacityClick(Sender: TObject);
var
  TableIdx: Integer;
  TableName: string;
  TableProps: TtdbTableProperties;
  NewCapacityStr: string;
  NewCapacity: Integer;
begin
  if lstTables.Selected <> nil then
  begin
    // Retrieve the table index, stored in Data property of the item
    TableIdx := Integer(lstTables.Selected.Data);
    TableName := lstTables.Selected.Caption;
    // Get table properties; Name is not allocated, so it won't be filled
    TableProps.Name := nil;
    if TDBTableGetProperties(FIndex, TableIdx, TableProps) then
    begin
      NewCapacityStr := IntToStr(TableProps.Capacity);
      if InputQuery('Set Table Capacity', 'Enter new capacity value', NewCapacityStr) then
      begin
        try
          NewCapacity := StrToInt(NewCapacityStr);
          if (NewCapacity < 0) or (NewCapacity >= 65535) then
            raise EConvertError.Create('Invalid capacity value');
          if TDBTableChangeCapacity(FIndex, PChar(TableName), NewCapacity) then
          begin
            UpdateTableListItem(lstTables.Selected);
            FModified := True;
            CheckEnabled;
            ...
            MessageDlg('Unexpected error while changing table capacity', mtError, [mbOk], -1);
        except
          on E: EConvertError do
            MessageDlg('The capacity must be a number in 0-65534 range', mtError, [mbOk], -1);
        end;
      end;
    end;
  end;
...
procedure TfrmMain.GridPopupMenuPopup(Sender: TObject);
var
  RecNo: Integer;
  Code: Integer;
begin
